...
def prep_words(name, word_list_ini):
    ...
    print("length of new work_list = {}".format(len(word_list)))
    return word_list
...
def trigram_filter(filter_1, trigrams_filtered):
    ...
    print("# of choices after fitler_2 = {}".format(len(filter_2)))
    return filter_2
...
def view_by_letter(name, filter_3):
    ...
    try_again = input("Try again? (Press Enter, else anyt other key to Exit):")
    if try_again.lower() == '':
        view_by_letter(name, filter_3)
    else: 
        sys.exit() 
